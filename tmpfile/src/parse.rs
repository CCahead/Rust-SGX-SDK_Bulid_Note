    ...
    while let Some((i, ch)) = chars.next() {
        match ch {
            ...
            '\\' => match chars.next() {
                ...
                Some((_, 'n')) | Some((_, 'r')) | Some((_, 't')) | Some((_, '\\')) | Some((_, '\'')) | Some((_, '"')) | Some((_, '0')) => {}
                Some((_, 'u')) => {
    ...
    while let Some((offset, b)) = bytes.next() {
        match b {
            ...
            b'\\' => match bytes.next() {
                ...
                Some((_, b'n')) | Some((_, b'r')) | Some((_, b't')) | Some((_, b'\\')) | Some((_, b'0')) | Some((_, b'\'')) | Some((_, b'"')) => {}
                Some((newline, b)) if b == b'\n' || b == b'\r' => {
                    input = input.advance(newline + 1);
    ...
    while let Some((i, ch)) = chars.next() {
        match ch {
            ...
            '\\' => match chars.next() {
                ...
                Some((_, 'n')) | Some((_, 'r')) | Some((_, 't')) | Some((_, '\\')) | Some((_, '\'')) | Some((_, '"')) => {}
                Some((_, 'u')) => {
...
fn byte(input: Cursor) -> Result<Cursor, Reject> {
    ...
    let ok = match bytes.next().map(|(_, b)| b) {
        Some(b'\\') => match bytes.next().map(|(_, b)| b) {
            ...
            Some(b) if b == b'n' || b == b'r' || b == b't' || b == b'\\' || b == b'0' || b == b'\'' || b == b'"' => true,
            _ => false,
        },
        b => b.is_some(),
    };
...
fn character(input: Cursor) -> Result<Cursor, Reject> {
    ...
    let ok = match chars.next().map(|(_, ch)| ch) {
        Some('\\') => match chars.next().map(|(_, ch)| ch) {
            ...
            Some(c) if c == 'n' || c == 'r' || c == 't' || c == '\\' || c == '0' || c == '\'' || c == '"' => true,
            _ => false,
        },
        ch => ch.is_some(),
    };
    ...
    I: Iterator<Item = (usize, char)>,
{
    ...
    if let Some((_, ch)) = chars.next() {
        match ch {
            '0'..='7' => {}, // 匹配 '0' 到 '7'
            '0'..='9' | 'a'..='f' | 'A'..='F' => {}, // 匹配 '0' 到 '9', 'a' 到 'f', 和 'A' 到 'F'
            _ => return Err(Reject), // 如果不匹配，返回错误
        }
    } else {
        return Err(Reject); // 如果没有字符，返回错误
    }
    ...
    I: Iterator<Item = (usize, u8)>,
{
    for _ in 0..2 {
        if let Some((_, ch)) = chars.next() {
            match ch {
                b'0'..=b'9' | b'a'..=b'f' | b'A'..=b'F' => {}, // 符合条件
                _ => return Err(Reject), // 如果不匹配，返回错误
            }
        } else {
            return Err(Reject); // 如果没有更多字符，返回错误
        }
    }
    Ok(())
}
...
where
    I: Iterator<Item = (usize, char)>,
{
    let mut chars = chars.take(2).map(|(_, ch)| ch).collect::<Vec<_>>();

    if chars.len() != 2 || chars.iter().all(|&ch| ch == '0') {
        Err(Reject)
    } else {
        Ok(())
    }
